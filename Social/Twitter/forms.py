...
class TweetForm(forms.ModelForm):
    body=forms.CharField(required=True,
                         widget=forms.Textarea(attrs={'placeholder': 'What\'s happening?','class':'form-control'}),
                         label="")
    class Meta:
        model=Tweet
        exclude=['user','likes']
...
class ProfilePicForm(forms.ModelForm):
    profile_image = forms.ImageField(label="Profile Image", required=False, error_messages = {'invalid':("Image files only")})
    class Meta:
        model=Profile
